...
import { GitAuthStatus, GitRepository, GitServerConfig, GitServerCredentials, GitServerValidationResult, GitValidationResult, GitServerAuthStatus } from "@/shared/types/git-repository"
...
export class UnifiedGitService {
     ...
     */
    /**
     * Create a new repository with auto-authentication and URL-based server detection
     */
    async createRepository(config: CreateRepositoryConfig, serverId?: string): Promise<GitRepository> {
        ...
        const credentials = await this.gitAuthService.getServerCredentials(actualServerId);
        if (!credentials) {
        ...
        const createdRepo = await this.gitService.createRepository(config, {
            method: credentials.method,
            token: credentials.token,
            username: credentials.username,
            password: credentials.password,
    ...
    /**
     * Detect Git provider type from base URL
    ...
    private detectProviderFromUrl(baseUrl: string): 'github' | 'gitlab' | 'gitea' | 'bitbucket' {
        ...
        // This can be enhanced with API endpoint detection if needed
        return 'gitea';
    }
