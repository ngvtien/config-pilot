...
describe('TDD - SchemaFieldSelectionModal Bug Reproduction', () => {
    ...
    describe('FAILING TEST - filterProperties spec field inclusion bug', () => {
        ...
        /**
         * This test reproduces the exact bug from the console log
         * It MUST FAIL with the current implementation
         */
        it('SHOULD FAIL - spec field should be included when spec.project is selected', () => {
            // Scenario: User selects "spec.project" which gets normalized to "project"
            const selectedPaths = new Set(['project']);
            
            // Simulate the filterProperties logic for the spec field
            ...
            // Current broken logic: Check if spec field is directly selected
            const isSelected = pathVariations.some(variation => selectedPaths.has(variation));
            
            // Current broken logic: Generic hasSelectedChildren check
            ...
            // The bug: This should be true but will be false with current implementation
            const shouldIncludeSpecField = isSelected || hasSelectedChildrenGeneric;
            
            // THIS TEST WILL FAIL - demonstrating the bug
            expect(shouldIncludeSpecField).toBe(true); // This will fail because hasSelectedChildrenGeneric is false
            
            // Additional assertions to show the problem
            expect(isSelected).toBe(false); // spec itself is not selected
            expect(hasSelectedChildrenGeneric).toBe(false); // This is the bug - should be true
        });
    ...
    describe('FAILING TEST - Complete filterProperties integration', () => {
         ...
         */
        it('SHOULD FAIL - broken filterProperties excludes spec field when spec.project is selected', () => {
            ...
            // THIS WILL FAIL - spec field should be included but isn't
    ...
    describe('Edge Cases - TDD', () => {
        ...
        it('SHOULD FAIL - multiple spec children selected but spec field excluded', () => {
            ...
            // Simulate broken logic
            const pathVariations = ['spec'];
            const hasSelectedChildrenBroken = Array.from(selectedPaths).some(selectedPath =>
                pathVariations.some(variation =>
                    selectedPath.startsWith(`${variation}.`) ||
                    selectedPath.startsWith(`${variation}[]`)
                )
            ...
            // THIS WILL FAIL - should be true but is false
            expect(hasSelectedChildrenBroken).toBe(true);
        });
